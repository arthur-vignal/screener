...
type EncryptedTextProps = {
   ...
   */
  flipDelayMs?: number
  /** Delay before the animation begins once in view, in milliseconds. */
  startDelayMs?: number
...
export const EncryptedText: React.FC<EncryptedTextProps> = ({
  ...
  // Avoid hydration mismatch: render the real text on the server / first paint,
  // and only begin scrambling once mounted on the client.
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      const currentRevealCount = Math.max(
        0,
  ...
  return (
    <motion.span ref={ref} className={cn(className)} aria-label={text} role="text">
      {text.split("").map((char, index) => {
        const isRevealed = !mounted || index < revealCount
        ...
        return (
          <span key={index} className={cn(isRevealed ? revealedClassName : encryptedClassName)} aria-hidden="true">
            {displayChar}
          </span>
        )
      })}
