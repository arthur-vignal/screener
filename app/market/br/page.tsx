...
const SECTIONS = [
  {
    key: "stock",
    title: "Ações",
    subtitle: "Ações listadas na B3 (carteira teórica vigente + full list).",
    href: "/market",
    accent: "text-brand-deep",
  },
  {
    key: "fii",
    title: `Fundos Imobiliários ${BR}`,
    subtitle: "Fundos de investimento imobiliário negociados na B3.",
    href: "/market/fiis",
    accent: "text-warning",
  },
  {
    key: "etf",
    title: `ETFs ${BR}`,
    subtitle: "ETFs listados na B3 (rastreiam índices ou setores).",
    href: "/market/etfs",
    accent: "text-brand-deep",
  },
  {
    key: "bdr",
    title: `BDRs ${BR}`,
    subtitle: "Brazilian Depositary Receipts — ações estrangeiras em Reais.",
    href: "/market/bdrs",
    accent: "text-muted",
  },
];
      ...
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        <div>
          ...
          <p className="text-body text-sm mt-1 max-w-2xl">
            Visão geral dos mercados listados na B3: ações, FIIs, ETFs e BDRs.
            Cada card mostra os 5 ativos mais relevantes da categoria; clique
...
function PreviewSection({
  ...
  useEffect(() => {
    ...
    // 1. Pull a list of symbols from /api/assets/list?exchange=b3&type=<key>
    // 2. Pull quotes for those symbols via /api/assets/quote

    const offsetLimit = section.key === "stock" ? { offset: 0, limit: 5 } : { offset: 0, limit: 5 };
    const params = new URLSearchParams({
      exchange: "b3",
      offset: String(offsetLimit.offset),
      limit: String(offsetLimit.limit),
    });
    if (section.key !== "stock") {
      params.set("type", section.key);
    }
  ...
  return (
    <div className="border border-hairline-strong bg-surface-elevated p-5 hover-lift">
      ...
      {!loaded ? (
        ...
        <div className="label-s text-muted py-4 text-center">
          Sem dados.
        </div>
      ) : (
